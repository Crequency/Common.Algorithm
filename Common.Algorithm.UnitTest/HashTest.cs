using Common.Algorithm.Interop;
...
namespace Common.Algorithm.UnitTest
{
    [TestClass]
    public class 哈希测试
    {
        ...
        public async void 环境安装测试()
        {
            ...
            Directory.Delete(Path.GetFullPath($"{Interop.Environment.DllPath}"), true);
            Console.WriteLine(Path.GetFullPath($"{Interop.Environment.DllPath}"));
            ...
            Directory.Delete(Path.GetFullPath($"{Interop.Environment.DllPath}"), true);
            if (!Interop.Environment.CheckEnvironment())
                await Interop.Environment.InstallEnvironmentAsync();
            Thread.Sleep(3000);
            Assert.IsTrue(Interop.Environment.CheckEnvironment());
        }
