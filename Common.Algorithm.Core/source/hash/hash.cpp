...
namespace Common::Algorithm::Core::Hash {
    ...
    inline i32 mix_2(i32 a, i32 b) {
        ...
        Math::maxin(&max, &min, 6, ea1, ea2, ea3, eb1, eb2, eb3);

        mid1 = Math::mid(ea2, ea3, eb1);
        mid2 = Math::mid(ea3, eb1, eb2);
    ...
    inline i32 mix_3(i32 a, i32 b, i32 c) {
        ...
        Math::maxin(&t_max, &t_min, 3, a, b, c);
    ...
    EXPORTED void hash_str(uchar *src, uchar *rst, int length) {
        ...
        memset(rst, '0', sizeof(uchar) * hash_length);                  //  初始化结果数组
