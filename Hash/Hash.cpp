...
namespace calg{
    ...
    EXTERN_API void hash_str(uchar *src, uchar *rst, int length){
        ...
        i32 **ato = new i32 * [hash_length];                            //  Grava 函数矩阵

        memset(mid, 0, sizeof(i32) * hash_length);                      //  初始化中间运算结果数组
        for (int i = 0; i < hash_length; ++i){
            ato[i] = new i32[hash_length];                              //  声明并创建 Grava 矩阵
            memset(ato[i], 0, sizeof(i32) * hash_length);               //  初始化 Grava 矩阵
        }
        ...
        if (length == hash_length)                                      //  长度刚好, 直接填充
            ...
            for (i32 i = 0; i < hash_length; ++ i){
                mid[i] = tar.front(); tar.pop();
            }
        }

        /* 中间运算结果 */
        for (i32 x1 = 0, x2 = 1023, x3 = 1024, x4 = 2047;
             x1 <= hash_length && x4 >= 0 && x1 <= x2 && x4 >= x3;
             ++ x1, ++ x3, -- x2, -- x4){
            mid[x1] += mid[x2] * mid[x3] - mid[x4] ^ mid[x1];
            mid[x2] = mix_3(mid[x1], mid[x3], mid[x4]);
            mid[x3] = mix_3(mid[x1], mid[x2], mid[x4]);
            mid[x4] += mid[x3] * mid[x2] - mid[x1] ^ mid[x4];
            if (mid[x1] & 1) ++ x1;
            if (mid[x2] & 1) -- x2;
            if (mid[x3] & 1) ++ x3;
            if (mid[x4] & 1) -- x4;
            i64 sum = (i64)(mid[x1] + mid[x2] + mid[x3] + mid[x4]);
            i64 mul = (i64)(mid[x1] * mid[x2] * mid[x3] * mid[x4]);
            exp_1(sum * mul - sum - mul, &mid[x1], &mid[x2], &mid[x3], &mid[x4]);
        }

        for (i32 i = 0; i < hash_length; ++ i)                          //  拷贝中间运算结果到结果
            rst[i] = (i32)calg::abs((i64)mid[i]) % 255;

        for (i32 i = 0; i < hash_length; ++i){                          //  回收 Grava 函数矩阵
            delete[]ato[i]; ato[i] = NULL;
        }

        delete[]ato; ato = NULL;                                        //  回收 Grava 函数矩阵
